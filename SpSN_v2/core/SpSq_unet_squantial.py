...
from SpSN_v2.utils.spsqModule import GCA, CLI
...
class MinkUNetEncoder_spsqNet(ResNetBase):
    # BLOCK = None
    # PLANES = None
    DILATIONS = (1, 1, 1, 1, 1, 1, 1, 1)
    LAYERS = (1, 1, 1, 1, 1, 1, 1, 1)
    ...
    # Once data is processed, call clear to reset the model before calling
    # initialize_coords
    def __init__(self, in_channels, out_channels, BLOCK=None, PLANES=None, D=3):
        assert BLOCK
        assert PLANES
        self.BLOCK = BLOCK
        self.PLANES = PLANES
    ...
    def forward(self, x, surppot):
        ...
        out = self.relu(out)
        out_b1p2 = self.block1(out)
        out_b1p2 = self.GCA(out_b1p2, surppot[1])

        out = self.conv2p2s2(out_b1p2)
        out = self.bn2(out)
        out = self.relu(out)
        out_b2p4 = self.block2(out)
        out_b2p4 = self.GCA(out_b2p4, surppot[2])

        out = self.conv3p4s2(out_b2p4)
        out = self.bn3(out)
        out = self.relu(out)
        out_b3p8 = self.block3(out)
        out_b3p8 = self.GCA(out_b3p8, surppot[3])
        ...
        out = self.relu(out)
        out = self.block4(out)
        out = self.CLI(out, surppot[4])
        # out = self.out_tune(out)
        return out_p1, out_b1p2, out_b2p4, out_b3p8, out


class MinkUNetEncoder_v2(ResNetBase):
    # BLOCK = None
    # PLANES = None
    DILATIONS = (1, 1, 1, 1, 1, 1, 1, 1)
    LAYERS = (1, 1, 1, 1, 1, 1, 1, 1)
    ...
    def refresh_p(self, net):
        ...
        p_dict = {
            k: v for k, v in net.state_dict().items() if k in self.state_dict()
        }
